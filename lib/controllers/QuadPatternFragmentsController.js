...
function QuadPatternFragmentsController(options) {
  ...
  var self = this;
  Object.keys(this._datasources).forEach(function (datasourceKey) {
    var datasource = self._datasources[datasourceKey];
    if (datasource.role === 'index') {
      if (self._datasources[''])
        throw new Error('Only one index datasource can exist.');
...
function (request, query, datasourceSettings) {
  ...
  var supportsQuads = datasourceSettings.datasource.supportedFeatures.quadPattern || false;
